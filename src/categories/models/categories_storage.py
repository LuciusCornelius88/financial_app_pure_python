...
from config import dumps_path, categories_dump_file
...
class Categories(UserDict):
    ...
    def __init__(self):
        super().__init__()
        self._dump_path = Path(dumps_path + categories_dump_file)
        self._empty_storage_return = 'No categories in the storage'
        self._change_log = []
...
#     storage = categories()
#     category = category({'name': 'name', 'init_balance': 1000})

#     storage.add(category)
#     print(storage.view_change_log())
#     print(storage.get(category.id))
#     new_storage = categories()
